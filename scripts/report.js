...
<meta name="description" content="${esc(title)}: App Store competitor research, fetched from public Apple endpoints."><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Geist:wght@400;500;600&family=Geist+Mono:wght@400;500&display=swap">
<style>${fs.readFileSync(path.join(__dirname, 'report.css'), 'utf8')}</style></head>
<body>${SPRITE}
<aside class="sidebar"><a class="brand" href="#overview"><img src="${MARK}" alt=""><span class="brand-name">App Store <em>Research</em></span></a><p class="brand-caption">${esc(title)}</p><div class="nav-label">THIS REPORT</div><nav aria-label="Report sections"><a href="#overview" class="nav-link selected">${icon('overview')}<span>Overview</span><span class="nav-no">—</span></a>${nav}</nav><div class="side-bottom"><div class="side-version"><span>${dlong(today)}</span></div></div></aside>
<div class="main">
<main class="page"><section id="overview"><div class="eyebrow">${icon('overview')} ${single ? 'APP REPORT' : 'IDEA REPORT'}</div><div class="heading-row"><div><h1>${h1}</h1></div></div><div class="meta"><span>${icon('calendar')} ${dlong(today)}</span><span>${icon('globe')} ${countries.map(c => ({ US: 'United States', IN: 'India', GB: 'United Kingdom', CA: 'Canada', AU: 'Australia', DE: 'Germany', FR: 'France', JP: 'Japan' })[c] || c).join(', ') || 'United States'}</span><span>${icon('reviews')} ${num(allReviews.length)} written reviews</span>${profiles.length > 1 ? `<span>${icon('apps')} ${profiles.length} apps</span>` : ''}</div></section>
