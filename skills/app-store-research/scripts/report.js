...
const TONES = {"market": "blue", "profile": "amber", "pricing": "green", "compare": "blue", "keywords": "violet", "insights": "rose", "explorer": "violet", "provenance": "grey"};
...
function build(run) {
  ...
  const kw = readJSON(path.join(run, 'keywords.json'));
...
<details class="extra" id="details-${p.id}"><summary>Description, what's new, privacy, similar apps${files.length > shown.length ? `, all ${files.length} screenshots` : ''} ${icon('chevron')}</summary><div class="extra-content">${files.length > shown.length ? `<div class="shots">${files.slice(shown.length).map(f => `<a href="${rel(path.join(sdir, f))}" target="_blank" rel="noopener"><img src="${rel(path.join(sdir, f))}" alt="" loading="lazy"></a>`).join('')}</div>` : ''}<div class="details-grid"><div><h3>Description</h3><p>${esc(p.description)}</p></div><div>${p.releaseNotes ? `<h3>What's new in ${esc(p.version)}</h3><p>${esc(p.releaseNotes)}</p>` : ''}<h3>Privacy</h3><p>${(p.privacy || []).map(t => `${esc(t.type)}: ${t.categories.map(esc).join(', ') || 'none'}`).join('<br>')}</p><h3>Apple's "you might also like"</h3><p>${p.similar.map(esc).join(', ')}</p><h3>Seller</h3><p>${esc(p.seller)}</p></div></div></div></details></div>`;
  ...
  if (kw && kw.terms.length) {
    const A = kw.apps;
    const cell = v => v == null ? '<span class="muted">–</span>' : v <= 3 ? `<b>#${v}</b>` : `#${v}`;
    const grid = `<div class="card table-wrap tw"><table class="stats-table keyword-table"><thead><tr><th style="text-align:left">SEARCH TERM</th>${A.map(a => `<th><a class="store-link" href="${esc(a.url || '')}" target="_blank" rel="noopener">${esc(a.short)}</a></th>`).join('')}<th style="text-align:left">APPLE SUGGESTS</th></tr></thead><tbody>${kw.terms.map(t => `<tr><td style="text-align:left"><b>${esc(t.term)}</b><span class="sub">${t.results} results</span></td>${A.map(a => `<td>${cell(t.positions[a.id])}</td>`).join('')}<td style="text-align:left" class="auto-cell">${t.autocomplete.slice(0, 6).map(x => `<span class="chip static">${esc(x)}</span>`).join(' ') || '<span class="muted">–</span>'}</td></tr>`).join('')}</tbody></table><div class="source-foot">Positions are the order of Apple's Search API for the ${kw.country.toUpperCase()} store, top 25. Suggestions are Apple's search autocomplete for that term.</div></div>`;
    const titles = `<div class="card table-wrap tw"><table class="stats-table"><thead><tr><th style="text-align:left">APP</th><th style="text-align:left">TITLE</th><th style="text-align:left">SUBTITLE</th></tr></thead><tbody>${A.map(a => `<tr><td style="text-align:left"><a class="store-link" href="${esc(a.url || '')}" target="_blank" rel="noopener">${esc(a.short)}</a></td><td style="text-align:left">${esc(a.name)} <span class="sub">${a.name.length} of 30 characters</span></td><td style="text-align:left">${a.subtitle ? esc(a.subtitle) + ` <span class="sub">${a.subtitle.length} of 30</span>` : '<span class="muted">none</span>'}</td></tr>`).join('')}</tbody></table></div>`;
    const others = kw.others.length ? `<div class="card table-wrap tw"><table class="stats-table"><thead><tr><th style="text-align:left">ALSO IN THE TOP 10</th><th>RATINGS</th><th style="text-align:left">FOR</th></tr></thead><tbody>${kw.others.slice(0, 10).map(x => `<tr><td style="text-align:left"><a class="store-link" href="${esc(x.url || '')}" target="_blank" rel="noopener">${esc(x.name)}</a></td><td>${num(x.ratings)}</td><td style="text-align:left">${x.terms.map(esc).join(' · ')}</td></tr>`).join('')}</tbody></table></div>` : '';
    sections.push({ id: 'keywords', label: 'Search terms', icon: 'keywords', h2: 'Search terms', html: (i) => secTop(i, 'Search terms') + `<div class="stack">${grid}${titles}${others}</div>` });
  }
