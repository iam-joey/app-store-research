...
|---|---|---|
| `find` | `shortlist.json`, `shortlist.csv` | `idea`, `country`, `terms`, `must`, `maxRatings`, `genreId`, `shortlist[]` (id, name, url, seller, rating, ratings, price, genre, version, updated, released, topFreeRank, topGrossingRank, hits, source, overCap), `chartsTop10` |
...
| `drop` | rewrites `profiles.json`, `reviews-summary.json`, `compare.json`; deletes `apps/<slug>-<id>/` | the dropped app is gone from every later command and from the report |
